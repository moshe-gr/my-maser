...
fun IncomeScreen(onEdit: (Float) -> Unit) {
    ...
    var lastIncome by remember { mutableStateOf(getLastHistoryByType(false)) }
        ...
        Column {
            OutlinedTextField(
                ...
                modifier = Modifier
                    ...
                    .onFocusChanged { expanded = it.isFocused && source.isNotBlank() },
                singleLine = true,
            ...
            if (expanded && suggestions.isNotEmpty()) {
                SuggestionsList(suggestions = suggestions) {
                    source = it
                    focusManager.moveFocus(FocusDirection.Down)
                    value = getAmountByName(it, false).toFloat()
